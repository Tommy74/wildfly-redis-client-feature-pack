...
public enum DistributableEjbSubsystemSchema implements SubsystemResourceXMLSchema<DistributableEjbSubsystemSchema> {
    ...
    ;
    static final Set<DistributableEjbSubsystemSchema> CURRENT = Set.of(VERSION_2_0, VERSION_2_0_COMMUNITY, VERSION_3_0_COMMUNITY);
    ...
    ResourceXMLChoice beanManagementChoice() {
        ResourceXMLChoice.Builder choiceBuilder = this.factory.choice()
                .withCardinality(XMLCardinality.Unbounded.REQUIRED)
                .addElement(this.infinispanBeanManagementElement());
        if (this.since(VERSION_3_0_COMMUNITY)) {
            choiceBuilder.addElement(this.redisBeanManagementElement());
        }
    ...
    NamedResourceRegistrationXMLElement redisBeanManagementElement() {
        return this.beanManagementElementBuilder(BeanManagementResourceRegistration.REDIS)
                .addAttribute(RedisBeanManagementResourceDefinitionRegistrar.REDIS_CONNECTION)
                .build();
    }
